defmodule FiftyTwo.ChallengeControllerTest do
  ...
  describe "with anonymous" do
    setup do
      Repo.insert!(%FiftyTwo.User{username: "username", password: "password", email: "test@test.localhost"})
      :ok
    end
  ...
  describe "with authorized user" do
    ...
    test "creates resource and redirects when data is valid", %{conn: conn, user: user} do
      conn = post conn, challenge_path(conn, :create), challenge: Map.put(@valid_attrs, "user_id", user.id)
      assert redirected_to(conn) == challenge_path(conn, :index)
      assert Repo.get_by(Challenge, @valid_attrs)
    end

    test "does not create resource and renders errors when data is invalid", %{conn: conn, user: user} do
      conn = post conn, challenge_path(conn, :create), challenge: Map.put(@invalid_attrs, "user_id", user.id)
      assert html_response(conn, 200) =~ "New challenge"
    end
    ...
    test "updates chosen resource and redirects when data is valid", %{conn: conn, user: user} do
      challenge = Repo.insert! %Challenge{user_id: user.id}
      conn = put conn, challenge_path(conn, :update, challenge), challenge: Map.put(@valid_attrs, "user_id", user.id)
    ...
    test "does not update chosen resource and renders errors when data is invalid", %{conn: conn, user: user} do
      challenge = Repo.insert! %Challenge{user_id: user.id}
      conn = put conn, challenge_path(conn, :update, challenge), challenge: Map.put(@invalid_attrs, "user_id", user.id)
      assert html_response(conn, 200) =~ "Edit challenge"
    end
